...
* B:构造方法注意事项
	* a:如果我们没有给出构造方法，系统将自动提供一个无参构造方法。创造对象的时候自动调用构造方法
	* b:如果我们给出了构造方法，系统将不再提供默认的无参构造方法。
		* 注意：这个时候，如果我们还想使用无参构造方法，就必须自己给出。建议永远自己给出无参构造方法 要自己给空参构造 
...
* A:案例演示
	* 通过一个案例引入static关键字。 static country 一次赋值全家能用
...
* A:static关键字的特点
	...
	* c:被类的所有对象共享
		...
		* 举例：
			...
			* 共性用静态,特性用非静态
	* d:可以通过类名调用
		* 其实它本身也可以通过对象名调用。(Person.country = "中国") 
		* 推荐使用类名调用。
...
* 静态变量也叫类变量  成员变量也叫对象变量（概念上 时间上 空间上 代码上）
* A:所属不同
	* 静态变量属于类，所以也称为为类变量
...
* A:制作一个工具类 类的所有方法都是静态的 必须把构造方法私有 目的是不让其他类创建本类对象
	* ArrayTool
	* 1,获取最大值
	* 2,数组的遍历
	* 3,数组的反转
###何时使用静态
	* 对于变量层 共性使用静态 特性使用非静态
	* 对于方法能加尽量加  如果方法没有访问任何的非静态方法和非静态变量 就静态
	* 如果一个类内部全部都是静态方法 那就没有必要创建对象 所以就私有构造函数
