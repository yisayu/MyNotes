...
* A:如何编译运行带包的类
	* a:javac编译的时候带上-d.即可
	...
	两层含义1,代表全类名,就是说类名改变了 2,代表该类一定是在该包下
	* b:通过java命令执行。
		* java 包名.HellWord
...
* B:结论
* 
					本类	 同一个包下(子类和无关类)	不同包下(子类)(子类可以访问父类 父类访问不了子类)	不同包下(无关类)
		private 	Y		
		默认			Y		Y
		protected	Y		Y							Y
		public		Y		Y							Y				Y
		...
		class Outer {
			...
			class Inner {
				...
				public void show() {
					...
					System.out.println(Outer.this.num); 内部类能访问外部类的成员 因为 调用了外部类的对象
				}
			}
		}
...
* A:案例演示(局部内部类只能在方法中访问 要在方法内部创建该局部内部类的对象 访问局部内部类的成员)
	* 局部内部类访问局部变量必须用final修饰
	* 局部内部类在访问他所在方法中的局部变量必须用final修饰,为什么?
		因为当调用这个方法时,局部变量如果没有用final修饰,他的生命周期和方法的生命周期是一样的,当方法弹栈,这个局部变量也会消失,那么如果局部内部类对象还没有马上消失想用这个局部变量,就没有了,如果用final修饰会在类加载的时候进入常量池,即使方法弹栈,常量池的常量还在,也可以继续使用

		但是jdk1.8自动给你加上了final
